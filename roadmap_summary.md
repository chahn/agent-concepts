...
Our foundation is built upon a modified [`smolagents` architecture](https://huggingface.co/docs/smolagents/), specifically extending a `ToolUseAgent` with the following capabilities:

  * **Standardized Persistent Tool Synthesis (Agent Skills):** The agent continuously expands its action space by compiling useful generated code into standard [Agent Skill directories](https://agentskills.io/specification) (`SKILL.md` instructions, executable `/scripts`, and context `/references`).
...
### Phase 1: Foundations & The Agent Skills Adapter
  ...
  * **Tasks:**
      * Build an adapter for `smolagents` (leveraging the [Hugging Face ecosystem](https://github.com/huggingface/smolagents)) that natively loads and executes standard Agent Skills via the `SKILL.md` format.
